module.exports = {
  HTML:function(title, list, body, control){
    ...
    list = list+'</ul>';
    return list;
  }, authorSelect:function(authors, author_id){ // sql에서 들고온 매개변수
    var tag = 0;
    var i = 0;
    ...
    return tag;
  }, authorTable : function(authors) { // sql에서 들고온 매개변수
      var tag='<table>';
      var i=0;
      while(i<authors.length) {
        tag+=`<tr> // 각각의 열은 td로 감쌈. table division, // 각각의 행은 tr로 감싸고
                ...
                </td> // 삭제는 링크 처리 XX(삭제 페이지로 가는건 괜찮), 반드시 폼으로!
              </tr>`
        i++;
      }
      tag+='</table>';
