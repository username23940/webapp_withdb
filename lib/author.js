...
// 삭제 var _url = request.url;
// 삭제 var queryData = url.parse(_url, true).query;
// 공통 변수 -> topic.js처럼 함수 내부로 이동
...
exports.home = function(request, response) {
  ...
  db.query('SELECT * FROM topic', function(error, topics){ // 콜백함수의 형식(signature). sql의 결과가 topics에 담김. template.HTML의 매개변수에 들어가야하므로 모든 페이지 반드시 필요    
    ...
    db.query('SELECT * FROM author', function(error2, authors){     
      ...
      ``); // title, description 수정 -> 저자 목록으로. template.html의 control 부분(crud)
...
exports.create_process = function(request, response) {
  ...
  request.on('end', function(){
    ...
    db.query(`INSERT INTO author (name, profile) VALUES(?,?)`, [post.name, post.profile], function(error, result){ // authorSelect method에서 select 태그의 name 속성에 해당하는 값 작성(post. ~~) 
      // <- 클라에서 넘긴 값을 받을 때 name 속성으로!! 헷갈리기 쉽다!!!
      if(error) {
          throw error;
      }
...
exports.update = function(request, response) {
  db.query('SELECT * FROM topic', function(error, topics){
    ...
    db.query('SELECT * FROM author', function(error2, authors){     
      ...
      db.query(`SELECT * FROM author WHERE id=?`,[queryData.id] function(error3, author){ // queryData.id인 이유는 authors.home에 있는 template.authorTable의 td 태그 보면 authors[i].id로 넘기니까! update process도 동일!
        if(error3){
          throw error3;
        }
      var title = 'author';
      ...
      <input type="hidden" name="id" value="${queryData.id}"> // update할 땐 고유한 id 값 필요(실제 수정할 author와 다른 고윳값)
      <p><input type="text" name="name" placeholder="name" value="${author[0].name}></p> // topics에서 topic은 배열 반환되는 것처럼 author도 요소 하나인 배열 반환되기 때문에 [0] 사용
      <p>
