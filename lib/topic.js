...
exports.home = function(request, response) { // 여러개의 요소 export. req, res를 매개변수로 넣은 이유는 ?? 서버가 만든 요청·응답 객체를 이 함수에 “주입”하기 위해서
/* 
http.createServer는 다른 파일에서 만들어지고 있어요.
즉, 요청(request)과 응답(response) 객체는
서버가 생성된 “다른 파일”에서 전달받는 거예요.
이 파일은 “요청을 처리하는 함수만(라우트 핸들러)” 정의하는 모듈이에요.

http 모듈은 서버를 생성할 때만 필요하고
이 파일은 “이미 생성된 서버가 request를 넘겨줄 때” 호출되므로
굳이 http를 불러올 필요가 없습니다.

topic.js는 DB + 템플릿 처리만 신경씀
main.js는 요청/응답 처리 흐름만 담당
서로 역할이 섞이지 않아서 유지보수가 쉬움
*/
