    ...
    if(pathname === '/'){
     ...
     db.query('SELECT * FROM author', function(error2, authors){ // 저자 선택 기능 추가
      ...
      request.on('end', function(){
          ...
          db.query(`INSERT INTO topic (title, description, created, author_id VALUES(?,?, NOW(), ?)`, [post.title, post.description, post.author], function(error, result){ // authorSelect method에서 select 태그의 name 속성에 해당하는 값 작성
              if(error) {
                  throw error;
              }
